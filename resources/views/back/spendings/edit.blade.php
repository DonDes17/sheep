    ...
    <div class="container col-lg-offset-1 col-lg-10">
        ...
        <form action="{{route('spending.update', $spending->id)}}" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="title" id="title" placeholder="Titre" value="{{$spending->title}}">
                @if($errors->has('title')) <span class="error">{{$errors->first('title')}}</span> @endif
            </div>
            <div class="form-group">
                <label for="price">Prix</label>
                <input type="text" class="form-control" name="price" id="price" placeholder="Price" value="{{$spending->price}}">
                @if($errors->has('price')) <span class="error">{{$errors->first('price')}}</span> @endif
            </div>
            <div class="form-group">
                <label for="describe">Description</label>
                <input type="text" class="form-control" name="description" id="describe" placeholder="Description" value="{{$spending->description}}">
                ...
                <label>
                   @foreach($users as $id => $name)
                        <input type="checkbox" class="form-check-input" name="users[]" value="{{$id}}"
                               @if(in_array($id, $checkedUsers) ) checked @endif>
